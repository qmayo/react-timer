...
export const getAvg = (solves: Array<PuzzleSolve>): number | null => {
  ...
  if (solves) {
    if (solves.length === 1) {
      ...
      return dnfs > 0 ? -1 : solves.map((solve) => solve.time).reduce((a, b) => a + b) / solves.length;
    } else {
          ...
          return -1
            ...
            for (let i = 0; i < excludedTimes; i++) {
              if (dnfs > 0) {
                ...
                const min = Math.min(...(
                  clonedSolves.filter((solve) => solve.time < 0 ? false : true).map((solve) => solve.time)
                ));
                clonedSolves.splice(
            ...
            return clonedSolves.map((solve) => solve.time).reduce((a, b) => a + b) / clonedSolves.length;
        }  
     } else {
        for (let i = 0; i < excludedTimes; i++) {
          const min = Math.min(...(clonedSolves.map((solve) => solve.time)));
